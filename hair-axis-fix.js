...
function align(){
  ...
  if(character&&modular){
    ...
    if(head&&modular.parent===head){
      ...
      const offsetWorld=new THREE.Vector3(0,charH*.012,0);
      ...
      if(modular!==last){console.info('[GG HAIR AXIS] Hair frame scale corrected',{worldScale:worldScale.toArray(),localScale:modular.scale.toArray(),position:modular.position.toArray()});last=modular}
    }
  }
  requestAnimationFrame(align);
}
requestAnimationFrame(align);
console.info('[GG HAIR AXIS] Character-aligned + FBX scale-corrected hair frame armed.');